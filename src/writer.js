...
function output(file, content) {
    //windows support
    var fullPath = OUTPUT_DIR + file.replace(':', '-');
...
exports.makeDir = function (p) {
    //hack: first function needs to initialize variables.
    OUTPUT_DIR = nconf.get('outputDir');
    var parts = p.split(/\//);
    var path = ".";
...
exports.copyTemplate = function (filename) {
    return exports.copy('./src/templates/' + filename, filename);
};
...
exports.copyDir = function copyDir(dir, destDir) {
    dir = path.normalize(dir);
    destDir = OUTPUT_DIR + destDir;
    qfs.exists(destDir).then(function (exists) {
        if (exists) {
            qfs.removeDirectory(destDir).then(function () {
                return qfs.copyTree(dir, destDir);
            });
        }
        else {
            return qfs.copyTree(dir, destDir);
...
exports.merge = function (srcs, to) {
    return merge(srcs.map(function (src) {
